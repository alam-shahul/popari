...
class SpiceMixPlus:
    ...
    def __init__(
        ...
        if betas is None:
            betas = np.full(self.num_repli, 1/self.num_repli)
        else:
            betas = np.array(betas, copy=False) / sum(betas)
        self.betas = betas
        if not prior_x_modes:
    ...
    def initialize(self, method='kmeans'):
        if method == 'kmeans':
            ...
            self.M, self.Xs = initialize_svd(self.K, self.Ys, context=self.context, random_state=self.random_state)
        else:
