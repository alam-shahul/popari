...
class SyntheticDataset:
    ...
    def reformat(self):
        ...
        for replicate in range(self.num_replicates):
            ...
            np.savetxt(self.data_directory / 'files'/ ('labels_{}.txt'.format(replicate)), self.cell_types[replicate], fmt='%d')
...
def synthesize_cell_expressions(points, distributions, cell_type_definitions, mask_conditions, num_genes, num_cells, variance_y, signal_sigma_x=0.1, background_sigma_x=0.03, sigma_x_scale=1.0):
...
#             num_walkers = num_genes // 10
#             repetitions = len(partition) // num_walkers + 1
#             for repetition in range(repetitions):
#                 noisy_cell_type_compositions = sample_truncated_gaussian(variance_y, cell_type_composition, num_walkers, num_steps=100)
#                 Y[partition[repetition * num_walkers: (repetition + 1) * num_walkers], :num_genes] = noisy_cell_type_compositions[:len(partition)]
            if isinstance(variance_y, dict):
                cell_type_variance_y = variance_y[cell_type]
            else:
                cell_type_variance_y = variance_y

            noisy_cell_type_compositions = sample_truncated_gaussian(cell_type_variance_y, cell_type_composition, len(partition)+10, num_steps=100)
            Y[partition, :num_genes] = noisy_cell_type_compositions[:len(partition)]
...
#             fig, axes = plt.subplots(2, len(self.variance_y) // 2 + 1)
#             for cov, ax in zip(self.variance_y.values(), axes.flat):
#                 ax.imshow(cov)
        ...
        for replicate in range(self.num_replicates):
            ...
            p_i = sample_2D_points(self.num_cells, minimum_distance)
            A_i = generate_affinity_matrix(p_i, tau)
            Y_i, C_i = synthesize_cell_expressions(p_i, distributions, self.cell_type_definitions, mask_conditions, self.num_genes, self.num_cells, self.variance_y)

            
            self.S = gamma.rvs(len(self.cell_type_definitions), scale=parameters['lambda_s'], size=self.num_cells)
            ...
            # for cell in range(self.num_cells):
            # #     self.Y[replicate][:, cell] = sample_gaussian(variance_y, Y_i[:, cell])
            #     self.Y[replicate][:, cell] = sample_truncated_gaussian(variance_y, Y_i[:, cell], 1, num_steps=1000)
                
            nmf = NMF(n_components=self.num_eigenvectors, max_iter=2000)
            embeddings = nmf.fit_transform(self.Y[replicate].T)
        ...
        self.data_directory = Path(parent_directory)
        self.initialize_data_directory()
    ...
    def plot_cell_types(self, replicate=0, save_figure=False, colors=None):
        ...
        df = pd.DataFrame({'X': points[:, 0], 'Y': points[:, 1], 'cell_type': cell_types}).sort_values(by=['cell_type'])
    ...
    def plot_feature_by_cell_type(self, replicate=0, **kwargs):
        """Plot 1D distribution of feature for each cell type (to check that they are indeed truncated Gaussians).
        ...
        fig, axes = plt.subplots((len(unique_cell_types)))
        fig.set_figheight(15)
        fig.set_figwidth(5)
        for cell_type, ax in zip(unique_cell_types, axes.flat):
            index = np.where(C_i == cell_type)[0]
            distribution = gene_expression[index, :2]
            print(distribution)
            ax.set_title(cell_type)
            ax.scatter(distribution[:, 0], distribution[:, 1], **kwargs)
