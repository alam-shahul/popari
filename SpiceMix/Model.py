...
from loadData import load_expression, load_edges
...
class Model:
    def __init__(self, path2dataset, repli_list, use_spatial, neighbor_suffix, expression_suffix, K,
        ...
        self.loadDataset(neighbor_suffix=neighbor_suffix, expression_suffix=expression_suffix)
            ...
            for i in range(self.num_repli):
                ...
                if self.Es_empty[i]:
                    rXTs.append(pool.apply_async(estimateWeightsWithoutNeighbor, args=(
                        self.YTs[i],
                        self.M[:self.Gs[i]], self.XTs[i], self.prior_xs[i], self.sigma_yx_inverses[i],
                        self.X_constraint, self.dropout_mode, i,
                    )))
                else:
                    rXTs.append(pool.apply_async(estimateWeightsICM, args=(
                        self.YTs[i], self.Es[i],
                        self.M[:self.Gs[i]], self.XTs[i], self.prior_xs[i], self.sigma_yx_inverses[i], self.sigma_x_inverse,
