...
def get_parser():
    """Get argument parser for Popari hyperparameter declaration."""

    parser = argparse.ArgumentParser(description="Run the Popari algorithm.")
    ...
    parser.add_argument(
        ...
        required=True,
        default=10,
        help="Number of metagenes to use for all replicates.",
    )
    parser.add_argument(
        "--num_iterations",
        type=int,
        required=True,
        default=200,
        help="Number of iterations to run Popari.",
    )
    parser.add_argument(
        "--nmf_preiterations",
        type=int,
        default=5,
        help="Number of NMF preiterations to use for initialization.",
    )
    parser.add_argument(
        "--dtype",
        type=str,
        required=True,
        default="float64",
        help="Datatype to use for PyTorch operations. Choose between ``float32`` or ``float64``",
    )
    parser.add_argument(
        "--torch_device",
        type=str,
        required=True,
        help="keyword args to use of PyTorch tensors during training.",
    )
    parser.add_argument(
        "--initial_device",
        type=str,
        required=True,
        help="keyword args to use during initialization of PyTorch tensors.",
    )
    ...
    parser.add_argument(
        "--lambda_Sigma_x_inv",
        type=float,
        help="Hyperparameter to balance importance of spatial information.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help="Approximate rate at which spots are aggregated into bins.",
    )
    parser.add_argument(
        "--superresolution_lr",
        type=float,
        help="Initial learning rate for superresolution optimization.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--betas",
        type=json.loads,
        help="weighting of each dataset during optimization. Defaults to equally weighting each dataset",
    )
    parser.add_argument(
        "--prior_x_modes",
        type=json.loads,
        help="family of prior distribution for embeddings of each dataset",
    )
    ...
    parser.add_argument(
        "--lambda_Sigma_bar",
        type=float,
        help="hyperparameter to constrain spatial affinity deviation in differential case.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help="convergence tolerance during optimization of ``Sigma_x_inv``",
    )
    parser.add_argument(
        "--spatial_affinity_constraint",
        type=str,
        help="method to ensure that spatial affinities lie within an appropriate range",
    )
    parser.add_argument(
        "--spatial_affinity_centering",
        type=bool,
        help="if set, spatial affinities are zero-centered after every optimization step",
    )
    parser.add_argument(
        "--spatial_affinity_scaling",
        type=float,
        help="magnitude of spatial affinities during initial scaling. Default ``10``",
    )
    parser.add_argument(
        "--spatial_affinity_regularization_power",
        type=int,
        help="exponent controlling penalization of spatial affinity magnitudes. Default ``2``",
    )
    parser.add_argument(
        "--embedding_mini_iterations",
        type=int,
        help="number of mini-iterations to use during each iteration of embedding optimization. Default ``1000``",
    )
    parser.add_argument(
        "--embedding_acceleration_trick",
        type=bool,
        help="if set, use trick to accelerate convergence of embedding optimization. Default ``True``",
    )
    parser.add_argument(
        "--embedding_step_size_multiplier",
        type=float,
        help="controls relative step size during embedding optimization. Default ``1.0``",
    )
    parser.add_argument(
        "--use_inplace_ops",
        type=bool,
        help="if set, inplace PyTorch operations will be used to speed up computation",
    )
    parser.add_argument(
        "--random_state",
        type=int,
        help="seed for reproducibility of randomized computations. Default ``0``",
    )
    parser.add_argument(
        "--verbose",
        type=int,
        help="level of verbosity to use during optimization. Default ``0`` (no print statements)",
    )
...
def main():
    ...
    trainer.train()
    trainer.superresolve(n_epochs=superresolution_epochs)
