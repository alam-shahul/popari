...
class DomainKDTree(KDTree):
    """Wrapper of KDTree for spatial domain simulation.

    Using these, can query a "landmark" KD tree in order to obtain spatial
    domain labels for simulation.

    """

    def __init__(self, landmarks: np.ndarray, domain_labels: Sequence[Union[int, str]]):
        super().__init__(data=landmarks)
    ...
    def query(self, coordinates: np.ndarray):
        ...
        """
        distances, indices = super().query(coordinates)

        return self.domain_labels[indices]


class DomainCanvas:
    """Annotate spatial domains on a reference ST dataset and use to define
    domains for simulations."""
    ...
    def render_reference_points(self):
        ...
        if len(self.domains) > 0:
            kdtree = self.generate_domain_kd_tree()
            color = [self.colors[domain_name] for domain_name in np.squeeze(kdtree.query(self.points))]
        else:
            self.canvas.stroke_style = "gray"
            color = None

        self.draw_points(x, y, color=color)
    ...
    def draw_points(self, x, y, color=None, fill=False):
        if color is None:
            self.canvas.stroke_circles(x, y, self.radius)
        else:
            self.canvas.stroke_styled_circles(x, y, self.radius, color=color)
    ...
    def bind_canvas(self):
        ...
        def handle_mouse_down(x, y):
            ...
            approximate_point = np.around((x / self.scaling_factor, y / self.scaling_factor), decimals=self.precision)
            self.domains[self.current_domain].append(approximate_point)
    ...
    def set_color(self, domain_name: str):
        ...
        self.canvas.stroke_style = color_string
        self.canvas.fill_style = color_string
...
class MetageneCanvas(DomainCanvas):
    def convert_metagenes_to_cell_types(self):
        """Assuming that the previous domain annotations are metagenes, converts
        these to unique cell types based on presence of coannotated
        metagenes."""
