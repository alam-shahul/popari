...
def test_popari_trainer(test_datapath, shared_model):
    train_parameters = TrainParameters(
        nmf_iterations=0,
        iterations=1,
        savepath=(test_datapath / f"popari_trainer_test.h5ad"),
    )
...
def test_nmf_trainer(test_datapath, shared_model):
    train_parameters = TrainParameters(
        nmf_iterations=1,
        iterations=0,
        savepath=(test_datapath / f"nmf_trainer_test.h5ad"),
    )
...
def test_mlflow_trainer(test_datapath, shared_model):
    try:
        ...
        pytest.skip("`[mlflow]` dependencies must be installed for `MLFlowTrainer` test.")
