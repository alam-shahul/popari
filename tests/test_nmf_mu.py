...
def spicemix_without_neighbors():
    ...
    for iiter in trange(5):
        obj.estimate_weights(iiter=iiter, use_spatial=[False]*obj.num_repli, backend_algorithm="mu")
        obj.estimate_parameters(iiter=iiter, use_spatial=[False]*obj.num_repli)

    return obj

def test_Sigma_x_inv_no_neighbors_mu(spicemix_without_neighbors):
    Sigma_x_inv = spicemix_without_neighbors.Sigma_x_inv.detach().cpu().numpy()
    np.save("../tests/test_data/synthetic_500_100_20_15_0_0_i4/outputs/Sigma_x_inv_no_neighbors_mu.npy", Sigma_x_inv)
    test_Sigma_x_inv = np.load("../tests/test_data/synthetic_500_100_20_15_0_0_i4/outputs/Sigma_x_inv_no_neighbors_mu.npy")
    assert np.allclose(test_Sigma_x_inv, Sigma_x_inv)
    
def test_M_no_neighbors_mu(spicemix_without_neighbors):
    M = spicemix_without_neighbors.M.detach().cpu().numpy()
    np.save("../tests/test_data/synthetic_500_100_20_15_0_0_i4/outputs/M_no_neighbors_mu.npy", M)
    test_M = np.load("../tests/test_data/synthetic_500_100_20_15_0_0_i4/outputs/M_no_neighbors_mu.npy")
    assert np.allclose(test_M, M)
    
def test_X_0_no_neighbors_mu(spicemix_without_neighbors):
    X_0 = spicemix_without_neighbors.Xs[0].detach().cpu().numpy()
    np.save("../tests/test_data/synthetic_500_100_20_15_0_0_i4/outputs/X_0_no_neighbors_mu.npy", X_0)
    test_X_0 = np.load("../tests/test_data/synthetic_500_100_20_15_0_0_i4/outputs/X_0_no_neighbors_mu.npy")
    assert np.allclose(test_X_0, X_0)
    
def test_louvain_clustering_no_neighbors_mu(spicemix_without_neighbors):
    df_meta = []
    path2dataset = Path('../tests/test_data/synthetic_500_100_20_15_0_0_i4')
    repli_list = [0]
    for r in repli_list:
