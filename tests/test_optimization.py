...
def test_compute_loss_and_gradient(test_data):
    ...
    def compute_loss_and_gradient(M):
        ...
        datasets = [
            dataset for (use_replicate, dataset) in zip(replicate_mask, param_optimizer.datasets) if use_replicate
        ]
        ...
        if param_optimizer.lambda_M > 0 and M_bar is not None:
            differential_regularization_quadratic_factor = param_optimizer.lambda_M * torch.eye(
                K,
                **param_optimizer.context,
            )
        ...
        if param_optimizer.metagene_mode == "differential" and M_bar is not None:
            ...
            for group_M_bar in M_bar:
                differential_regularization_term += (
                    group_weighting * param_optimizer.lambda_M * (group_M_bar * group_M_bar).sum()
                )
...
def test_update_z_gd_nesterov(test_data):
    """Test update_z_gd_nesterov function from EmbeddingOptimizer with one
    step."""
    # Get test data
    Z = test_data["Z"].clone()
    S = test_data["S"].clone()
